...
[**Griffiths & Tenenbaum (2001):**]{.accent} the question $P(x \mid \text{random})$ alone isn't *Bayesian*. Some hypothesis has to explain $x$ — so "is it random?" only makes sense *relative to an alternative*: "or is some regularity in the world responsible?"
:::
...
[**Griffiths & Tenenbaum (2001):**]{.accent} $P(x \mid \text{random})$ 単体はベイズ的な問いではない。$x$ を説明する何らかの仮説が必要 — だから「ランダムか?」は*代替仮説に対して相対的にしか意味をなさない*: 「それとも世界の何らかの規則性が原因か?」
:::
...
::: {.lang-en}
[A **likelihood ratio.** With a uniform prior over the two hypotheses, this ratio *is* the posterior odds — the answer to "given $x$, is random or regular more likely?"]{.yellow}
...
::: {.lang-ja}
[**尤度比。** 2つの仮説に一様な事前を置くと、この比がそのまま事後オッズ — 「$x$ を見て、ランダムと規則のどちらがありそうか?」の答え。]{.yellow}
