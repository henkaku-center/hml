...
::: {.lang-en}
[Then $P(R_k = \text{H}) = \sigma(\lambda L_k)$. **A long run of H's drives $L_k$ negative** (every suffix already looks H-heavy), so the model strongly prefers T next — no free "switch preference" parameter needed.]{.yellow}
:::

::: {.lang-ja}
[次に $P(R_k = \text{H}) = \sigma(\lambda L_k)$。**Hの連続が長いと $L_k$ が負に振れる**（どの部分系列もすでにH偏重）ので、モデルは次にTを強く好む — 自由な「切替選好」パラメータ不要。]{.yellow}
:::
