...
[5 samples: $0.72, \; 0.89, \; 0.83, \; 0.64, \; 0.82$]{.dim}
:::

::: {.lang-ja}
$\text{Beta}(8, 3)$: 質量は **$0.5$ より上に集中**、ピークは $0.78$ あたり。中程度の自信 — 鋭くも平坦でもない。

[5サンプル: $0.72, \; 0.89, \; 0.83, \; 0.64, \; 0.82$]{.dim}
:::
...
## [Five Betas — sweep the parameters]{.lang-en}[5つのベータ — パラメータを動かす]{.lang-ja}

![](images/beta_grid.png){fig-align="center" width="92%"}

::: {.lang-en}
[**Two dials.** $\alpha/\beta$ ratio: where mass concentrates. $\alpha + \beta$: how confident.]{.yellow}
:::

::: {.lang-ja}
[**2つのダイヤル。** $\alpha/\beta$ 比: 質量の位置。$\alpha + \beta$: 自信の強さ。]{.yellow}
:::

::: {.notes}
The sweep. Walk through each panel:
  - Beta(1,1): uniform — the start case.
  - Beta(8,3): α > β → concentrated above 0.5. Bento prior territory.
  - Beta(50,50): large α+β → narrow around 0.5 (strong belief in fair).
  - Beta(2,2): small α+β with α=β → wide, gentle center on 0.5.
  - Beta(0.5, 0.5): U-shape — strong prior on extremes. Tanaka marbles.
Land the two-dial intuition: α/β ratio is *where*, α+β is *how confident*.
:::
...
[Prior:]{.accent} $\theta \sim \text{Beta}(7, 3)$   ← encodes "70/30 with low confidence"

[Data:]{.accent} 27 tonkatsu in 40 ($k = 27$, $n = 40$)

[Posterior:]{.accent} $\theta \mid D \sim \text{Beta}(7 + 27, \; 3 + 13) = \text{Beta}(34, 16)$
...
## [The reframe — likelihood ratio, not likelihood]{.lang-en}[再構築 — 尤度ではなく尤度比]{.lang-ja} {.smaller}
...
[*What competes with "random" depends on the stimulus*: for binary sequences, "regular" = local representativeness (~50/50 in each subsequence). For numbers, it's features like "prime" or "ends in 7." The regularity space is the modeling commitment.]{.dim}
:::

::: {.lang-ja}
[**尤度比** — すべての共役更新に現れる同じベイズ因子の対象。]{.yellow}

[*「ランダム」と競合するものは刺激次第*: 2値系列なら「規則」= 局所代表性（各部分系列が約50/50）。数字なら「素数」「7で終わる」などの特徴。規則空間がモデリングのコミットメント。]{.dim}
:::
...
priors over "random" vs "regular," it IS the likelihood ratio. The dim
caption packs in the "what is regular?" point that previously had its own
slide — flag verbally that the regularity-space choice is the paper's
weakest spot if students are interested.
:::
