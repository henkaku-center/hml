...
::: {.lang-en}
For each new response, sum log-likelihood-ratios over all suffix lengths back to step 1 — bias toward whichever symbol makes recent subsequences look more 50/50:
:::

::: {.lang-ja}
新しい反応ごとに、1ステップ前から $k{-}1$ ステップ前までのすべての部分系列で対数尤度比を合計 — 直近の部分系列がより50/50に見える方の記号にバイアス:
:::

$$
L_k \;=\; \sum_{i=1}^{k-1} \log \frac{P(\text{suffix with new symbol} \mid \text{random})}{P(\text{suffix with the other symbol} \mid \text{random})}
$$

::: {.lang-en}
[Push through a logistic with scale $\lambda$ → $P(R_k)$. **A long run makes every suffix look extreme**, so the model strongly favors the *other* symbol — no free "switch preference" parameter needed.]{.yellow}
:::

::: {.lang-ja}
[スケール $\lambda$ のロジスティックに通して $P(R_k)$。**連続が長いと各部分系列が極端に見える**ので、モデルは*もう一方*の記号を強く支持 — 自由な「切替選好」パラメータ不要。]{.yellow}
:::

::: {.notes}
~3 min. The model spec. Key conceptual moves to land:
...
5. λ is the model's ONE free parameter — controls how strongly the bias
   plays. G&T fit λ ≈ 0.6 to the Zenith data.
...
![](images/gt2001_zenith_fig1.png){fig-align="center" width="62%"}
