...
## [What "conjugate" means — formally]{.lang-en}[「共役」とは — 形式的に]{.lang-ja} {.bigger}

::: {.lang-en}
A **prior family** $\mathcal{F}$ is **conjugate** to a likelihood when:
:::

::: {.lang-ja}
**事前分布の族** $\mathcal{F}$ が尤度に対して**共役**であるとは:
:::
...
::: {.lang-en}
[$\mathcal{F}$ is a *set* of distributions — e.g. "all Gaussians" or "all Betas."]{.dim}

[The posterior has the same functional form as the prior — just with updated parameters.]{.dim}
:::

::: {.lang-ja}
[$\mathcal{F}$ は分布の*集合* — 例: 「すべてのガウス」または「すべてのベータ」。]{.dim}

[事後は事前と同じ関数形 — パラメータが更新されるだけ。]{.dim}
:::
...
## [Quick check — when does conjugacy *fail*?]{.lang-en}[クイックチェック — 共役性が*破綻*するのは?]{.lang-ja} {.bigger}

::: {.lang-en}
Prior on $\mu$: **bimodal** (mixture of two Gaussians). Likelihood: Gaussian. Posterior?
...
::: {.lang-ja}
$\mu$ の事前: **二峰性**（2つのガウスの混合）。尤度: ガウス。事後は?
...
::: {.notes}
Self-authored poll. Commit-before-reveal: students vote, then reveal next slide.
Goal: probe whether "conjugacy" lives in the prior family alone, or in the
prior+likelihood pair. The Gaussian likelihood preserves Gaussian priors, but
a mixture-of-Gaussians prior gives a mixture-of-Gaussians posterior — still
closed form. So the "family" being closed is what matters. Source: self-authored
for SP26, not from a SP25 quiz item (Quiz 2 had no conceptual MC items at this
level). Discussion target: 1 min.
:::

## [Quick check — answer]{.lang-en}[クイックチェック — 答え]{.lang-ja} {.biggerplus}

::: {.lang-en}
[**B. Bimodal Gaussian mixture.**]{.yellow}
...
[Conjugacy is a property of the *prior family × likelihood pair*, not the prior alone.]{.accent}
:::

::: {.lang-ja}
[**B. 二峰性のガウス混合。**]{.yellow}

各成分が独立にガウス-ガウス更新を行い、データへの当てはまりの良さで混合重みが再調整される。

[共役性は*事前族 × 尤度の対*の性質であり、事前単体の性質ではない。]{.accent}
:::

::: {.notes}
Land the punchline. The "family" in conjugacy can be richer than a single Gaussian
— mixtures of conjugate priors are themselves conjugate. (We'll see mixture
modeling proper in a later week.) 30s here.
:::
