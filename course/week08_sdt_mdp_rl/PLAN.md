...
- [x] **Post-lecture (2026-06-20):** rebuilt `course/assignments/rl/` (Assignment 4, due Jul 10). New core = diagnose-and-fix reward hacking (implement TD update → quantify the AF +14/lap cycle → design a potential Φ(s) and verify policy invariance). Three stencils — `rl_genjax.ipynb` (env as a `@gen` generative model, Q-learning learns by sampling it), `rl_python.ipynb`, `rl_nosoln.Rmd` — all driven by a shared `rl_gridworld.py` (env + widget-style static renderer + ipywidgets interactive explorer). Both solved notebooks execute clean; old stencils archived to `sp25_reference/`. Textbook MDP/RL chapters (intro2/21–22) already authored.
