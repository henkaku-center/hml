...
::: {.notes}
Slide 4: 'Bin width 2g. In the limit, the bars ARE the PDF (up to integration). Blue curve overlaid is the underlying Gaussian — the same one the data were drawn from. The integral formula is how we RECOVER probabilities from the density: probability is AREA, density is height.
\
