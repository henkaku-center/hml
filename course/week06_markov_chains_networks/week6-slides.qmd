...
::: {.lang-en}
[**Picture view:** Chibany's chain — states + labelled arrows (the same one from "Draw the habit").]{.dim}
:::
::: {.lang-ja}
...
stationary π — generalises beyond the 2-state Chibany chain. Read the matrix
...
::: {.notes}
The payoff: the machinery generalises. pi ≈ (0.42, 0.13, 0.45) — states 1 and 3
...
the **bridge** between the two clusters. Long-run behaviour is dictated by
...
is what links the latent walk to the observed behaviour. Worked example next.
~2 min.
:::
...
[Structure + process **jointly predict** behaviour — and they fit human data well, with no recall-specific machinery bolted on.]{.yellow}
:::
...
structure/process/behaviour frame. The "memoryless" point is the discussion
...
H1 ("general slowing") vs H2 ("optimal with fewer resources") in favour of H2.
~2 min.
:::
...
::: {.notes}
OPTIONAL. The inverse problem (network from behaviour). Frame the network as a
