...
- **Two figure slides — the Tenenbaum & Griffiths (2001) BBS integration construction.** (a) *One datum, many hypotheses*: a single observed point $x$ with a stack of overlapping consequential-interval hypotheses, each drawn at a thickness $\propto$ its strong-sampling likelihood $1/|h|$ (smaller interval → thicker bar). (b) *Sum the votes → the gradient*: for each novel $y$, sum the posterior weight of every hypothesis containing $y$; the discretized bars trace an **approximately exponential** decay — Shepard's universal law *derived* rather than assumed. Figures generated by `scripts/build_tg_integration_plot.py` → `images/tg_hypotheses.png`, `tg_gradient.png`.
