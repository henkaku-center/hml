...
- Build up in order (each step has a dark-theme figure from `scripts/build_continuous_concept_plots.py`):
  ...
  2b. **The 1-D gradient, built from votes** (own slide): the multi-example analog of the $y=x/x{+}1/x{+}2$ vote walk-through. *Figure `cc_1d_gradient.png`* — top panel: candidate intervals consistent with several examples $X$, thickness ∝ posterior; bottom panel: the posterior-weighted sum, flat over the data and decaying outside. Same equation as the discrete construction, only $\mathcal H$ changed.
  ...
  4c. **The result — human vs. model, no prior** (own slide): *Figure `tg_results.png`* — $d$ vs $r$, one curve per $n$, recreated dark-theme from Tenenbaum (1999) Fig 3a/3c. **Human (solid) and model (dashed) on the same axes.** Human curves saturate; the model with the *uninformative* prior is exactly linear in $r$ (Eq 3) — it gets the $n$-ordering but **over-extends**, motivating the fix. Model curves are computed from the paper's closed forms, not eyeballed.
  5. **The exponential prior.** The uninformative prior lets the rectangle run straight; an **exponential prior over rectangle size** fixes it. *Figure `cc_exp_prior.png`* + the density $p(s)=\lambda e^{-\lambda s}$ — this is the **first time the class meets the exponential distribution**, so define it properly (rate $\lambda>0$, mean $1/\lambda$, monotonically decreasing). Speaker notes carry an anticipated student question — *how is the scale parameter determined?* (fit once to the population, σ≈5 in the 24-unit window; not learned per trial; the hierarchical-Bayes move is to put a hyperprior on it).
  5b. **With the prior — the fit** (own slide): *Figure `tg_results_prior.png`* — same axes, same human data; the model now carries the expected-size (exponential) prior, σ=5, computed from Eq (4). The dashed curves bend from linear to **saturating** and land on the human curves — the excellent fit the paper reports (Fig 3d). Likelihood + prior together; neither alone.
  - **Standing convention reinforced here:** whenever discussing a paper, show human **and** model results on the same axes — never a model curve alone, never a "the model reproduces it" claim without the human data beside it.
