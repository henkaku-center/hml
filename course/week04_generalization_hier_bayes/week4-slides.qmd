...
## [Shepard's universal law]{.lang-en}[シェパードの普遍法則]{.lang-ja} {.midbig}
...
::: {.fragment}
*Psychological space* — not raw physical/stimulus space. The perceived
distance, after the mind has represented the stimulus.
:::
:::

::: {.lang-ja}
[Shepard (1987)]{.accent} — 種や領域を超えて、一般化の確率は
**心理的空間**における距離とともに**指数関数的に減衰**する。

::: {.fragment}
*心理的空間* — 生の物理的・刺激空間ではない。心が刺激を表現した後の、
知覚された距離。
:::
:::
...
Shepard's law is remarkably robust empirically. The puzzle it leaves: WHY
exponential? And what sets the psychological space? Today's Bayesian framework
will answer the first — exponential decay falls out of the posterior. ~2 min.
:::
...
$$p(y \in C \mid X) \;=\; \sum_{h \in \mathcal{H}} [\,y \in h\,]\; p(h \mid X)$$
...
[$[\,y \in h\,]$ — the indicator: $1$ if $y$ is in the set $h$, else $0$.]{.dim}
