...
::: {.column width="40%"}
::: {.lang-en}
**Top** — observe one datum $x$. Every interval that **contains $x$** is a
live hypothesis $h$. Thickness $= 1/|h|$, its strong-sampling likelihood;
flat prior → posterior $\propto$ likelihood.

**Bottom** — the vote: for each $y$, sum the posterior of every $h$ that
contains it.
$$p(y \in C \mid x) = \sum_{h} \mathbf{1}[\,y \in h\,]\, p(h \mid x)$$
:::

::: {.lang-ja}
**上** — 1つのデータ $x$ を観測。$x$ を**含む**すべての区間が生きた仮説
$h$。太さ $= 1/|h|$（強サンプリング尤度）; 平坦な事前 → 事後 $\propto$ 尤度。

**下** — 投票: 各 $y$ について、$y$ を含むすべての $h$ の事後確率を合計。
$$p(y \in C \mid x) = \sum_{h} \mathbf{1}[\,y \in h\,]\, p(h \mid x)$$
:::
:::
::::
...
::: {.column width="40%"}
::: {.lang-en}
Ask about $y = x$ — the datum itself.

[**Every** hypothesis contains $x$]{.yellow} — all 7 vote "yes".

The bar at $x$ sums *all* the posterior weight: the tallest possible vote.
:::

::: {.lang-ja}
$y = x$ — データそのものについて問う。

[**すべての**仮説が $x$ を含む]{.yellow} — 7つすべてが「はい」に投票。

$x$ のバーは*すべて*の事後確率を合計 — 可能な限り高い投票。
:::
:::
::::
...
::: {.column width="40%"}
::: {.lang-en}
Move one step away: $y = x + 1$.

[The two smallest intervals no longer reach $y$]{.yellow} — they drop out
(greyed). Only 5 of 7 vote.

Fewer votes → a **shorter** bar.
:::

::: {.lang-ja}
1歩離れる: $y = x + 1$。

[最小の2区間はもう $y$ に届かない]{.yellow} — 脱落（グレー）。7つ中5つだけ
が投票。
...
::: {.column width="40%"}
::: {.lang-en}
Two steps away: $y = x + 2$.

[Only the 2 widest intervals still reach $y$]{.yellow} — and those are the
*least* likely. The bar is short.

Sweep $y$ across every point → [the bars trace an **approximately
exponential** decay. Shepard's law, **derived**.]{.yellow}
:::

::: {.lang-ja}
2歩離れる: $y = x + 2$。

[最も広い2区間だけが $y$ に届く]{.yellow} — しかもそれらは*最も*尤度が低い。
バーは低い。

$y$ を全点で動かす → [バーは**ほぼ指数関数的**な減衰を描く。シェパードの
法則が**導出**された。]{.yellow}
:::
:::
::::
...
::: {.column width="48%"}
::: {.lang-en}
[$X = \{60\}$]{.accent}, strong sampling:
$$p(h \mid X) \propto \left(\tfrac{1}{|h|}\right)^{1}\!\! \cdot 0.5$$

- mult-10: $\tfrac{1}{10}$ · even: $\tfrac{1}{50}$
- Normalize → [**0.83 vs. 0.17**]{.yellow}

One example already **tilts** belief toward the smaller hypothesis — but
gently. Lots of mass still on "even".
:::

::: {.lang-ja}
[$X = \{60\}$]{.accent}、強いサンプリング:
$$p(h \mid X) \propto \left(\tfrac{1}{|h|}\right)^{1}\!\! \cdot 0.5$$

- 10の倍数: $\tfrac{1}{10}$ · 偶数: $\tfrac{1}{50}$
- 正規化 → [**0.83 対 0.17**]{.yellow}

例が1つでもう、小さい仮説へ信念が**傾く** — だが穏やかに。「偶数」にも
まだ多くの質量。
:::
:::
::::
