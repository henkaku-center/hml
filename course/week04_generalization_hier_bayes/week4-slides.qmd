...
## [Weak sampling — the data never moves you]{.lang-en}[弱いサンプリング — データは信念を動かさない]{.lang-ja} {.smaller}
...
Both hypotheses contain every example, so:
$$p(h \mid X) = \frac{1 \cdot 0.5}{1\cdot 0.5 + 1\cdot 0.5} = 0.5$$

[The posterior just **reverts to the prior** — $0.5$ here only because the
space has 2 hypotheses with a flat prior. No size principle, no suspicious
coincidence: weak sampling **can't** explain the number game.]{.yellow}
:::

::: {.lang-ja}
弱いサンプリング: 尤度は[**データを含む $h$ なら $1$**]{.accent}、それ以外は
$0$ — *$|h|$ に依存しない*。

両仮説とも全例を含むので:
$$p(h \mid X) = \frac{1 \cdot 0.5}{1\cdot 0.5 + 1\cdot 0.5} = 0.5$$

[事後確率は**事前確率に戻る**だけ — ここで $0.5$ なのは、空間が平坦な事前を
持つ2仮説だから。サイズ原理も怪しい偶然もない: 弱いサンプリングは数当て
ゲームを**説明できない**。]{.yellow}
:::
:::
::::
...
::: {.notes}
Weak sampling. The key teaching point — and the answer to "why is it 0.5":
under weak sampling the likelihood is size-blind (1 or 0), so the posterior is
just the renormalised prior over the hypotheses that survive. Here that is
0.5 / 0.5 — but ONLY because there are exactly two hypotheses and the prior is
flat. With five consistent hypotheses it would be 0.2 each. The robust claim:
under weak sampling the data carries no discriminating signal, so the
posterior never leaves the prior — no matter how many examples. That is why
the number game's behaviour (few examples → a rule) REQUIRES strong sampling.
~2 min.
:::
