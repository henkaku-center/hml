...
- [Welcome + Clusters walkthrough]{.lang-en}[ようこそ + クラスタ課題の説明]{.lang-ja}  [0:00]{.time}
- [The generalization problem]{.lang-en}[一般化の問題]{.lang-ja}  [0:10]{.time}
- [The Bayesian generalization framework + size principle]{.lang-en}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en}[長方形ゲーム + 数当てゲーム]{.lang-ja}  [0:40]{.time}
- [Break]{.lang-en}[休憩]{.lang-ja}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en}[学生発表 — ショウヘイ]{.lang-ja}  [1:15]{.time}
- [No Free Lunch]{.lang-en}[ノーフリーランチ]{.lang-ja}  [1:40]{.time}
- [Hierarchical Bayes + close]{.lang-en}[階層ベイズ + まとめ]{.lang-ja}  [1:50]{.time}
...
- [Welcome + Clusters walkthrough]{.lang-en .done}[ようこそ + クラスタ課題の説明]{.lang-ja .done}  [0:00]{.time}
- [The generalization problem]{.lang-en .highlight}[一般化の問題]{.lang-ja .highlight}  [0:10]{.time}
- [The Bayesian generalization framework + size principle]{.lang-en}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en}[長方形ゲーム + 数当てゲーム]{.lang-ja}  [0:40]{.time}
- [Break]{.lang-en}[休憩]{.lang-ja}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en}[学生発表 — ショウヘイ]{.lang-ja}  [1:15]{.time}
...
::: {.notes}
Don't analyze yet. Let two or three students answer out loud — most will say
...
color, line length, faces. Shepard called it *universal* for that reason.
...
- [Welcome + Clusters walkthrough]{.lang-en .done}[ようこそ + クラスタ課題の説明]{.lang-ja .done}  [0:00]{.time}
- [The generalization problem]{.lang-en .done}[一般化の問題]{.lang-ja .done}  [0:10]{.time}
- [The Bayesian generalization framework + size principle]{.lang-en .highlight}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja .highlight}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en}[長方形ゲーム + 数当てゲーム]{.lang-ja}  [0:40]{.time}
- [Break]{.lang-en}[休憩]{.lang-ja}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en}[学生発表 — ショウヘイ]{.lang-ja}  [1:15]{.time}
- [No Free Lunch]{.lang-en}[ノーフリーランチ]{.lang-ja}  [1:40]{.time}
...
## [The idea — concepts as hypotheses]{.lang-en}[考え方 — 概念を仮説として]{.lang-ja}
...
- A [**feature**]{.accent} is a hypothesis too: "has stripes" picks out a set of
  stimuli — generalization is just asking which feature-sets the examples imply
...
- シェパードの用語: [**結果的部分集合（consequential subset）**]{.accent}
  — 性質が「当てはまる」ものの部分集合
- [**特徴（feature）**]{.accent}も仮説である: 「縞模様がある」は刺激の集合を
  選び出す — 一般化とは例がどの特徴集合を含意するかを問うこと
...
that survived the data?" The feature bullet is the unifying point — a feature
("has stripes", "is poisonous") is itself a hypothesis: it carves out a set of
stimuli, so learning a feature and learning a concept are the same inference.
~2 min.
:::
...
::: {.column width="46%"}
::: {.lang-en}
[**The hypotheses.**]{.accent} Observe one datum $x$. Every interval
**containing $x$** is a live hypothesis $h$. Bar thickness $= 1/|h|$ (the
strong-sampling likelihood); flat prior, so posterior $\propto$ likelihood.

[**The vote.**]{.accent} For each candidate $y$, sum the posterior of every
$h$ that contains it:
$$p(y \in C \mid x) = \sum_{h} \mathbf{1}[\,y \in h\,]\, p(h \mid x)$$

[Next three slides: walk $y$ outward — $x$, $x{+}1$, $x{+}2$.]{.yellow}
:::

::: {.lang-ja}
[**仮説。**]{.accent} 1つのデータ $x$ を観測。$x$ を**含む**すべての区間が
生きた仮説 $h$。バーの太さ $= 1/|h|$（強サンプリング尤度）; 平坦な事前なので
事後 $\propto$ 尤度。
...
[次の3枚: $y$ を外へ動かす — $x$、$x{+}1$、$x{+}2$。]{.yellow}
:::
:::
::::
...
[**Who votes.**]{.accent} Every hypothesis was built to contain $x$, so
[**all 7 of 7 vote "yes"**]{.yellow}.

[**The bar.**]{.accent} The vote sums *all* the posterior weight —
$\sum_h p(h \mid x) = 1$. The **tallest bar the gradient can have**.

[**The baseline.**]{.accent} This peak is what every other $y$ is measured
against — step away and the bar can only fall.
:::

::: {.lang-ja}
[**問い。**]{.accent} 性質は $y = x$、観測したデータそのものに一般化するか?

[**誰が投票するか。**]{.accent} すべての仮説は $x$ を含むように作られたので、
[**7つ中7つすべてが「はい」に投票**]{.yellow}。

[**バー。**]{.accent} 投票は*すべて*の事後確率を合計 —
$\sum_h p(h \mid x) = 1$。**勾配が取りうる最も高いバー**。

[**基準点。**]{.accent} この頂点が他のすべての $y$ の比較対象 — 離れれば
バーは下がるしかない。
:::
:::
::::
...
::: {.column width="46%"}
::: {.lang-en}
[**The question.**]{.accent} Step one unit out: does the property generalize
to $y = x + 1$?

[**Who drops.**]{.accent} [The two smallest intervals no longer reach
$y$]{.yellow} — they gray out and vote "no". [**5 of 7 still vote
"yes".**]{.yellow}

[**The bar.**]{.accent} Fewer hypotheses contribute → a **shorter** bar than
at $x$.

[**Why it falls fast.**]{.accent} The dropouts are the **smallest**
intervals — by the size principle the **heaviest**-weight votes. Losing those
first is why the gradient decays steeply near $x$.
:::

::: {.lang-ja}
[**問い。**]{.accent} 1単位外へ: 性質は $y = x + 1$ に一般化するか?

[**誰が脱落するか。**]{.accent} [最小の2区間はもう $y$ に届かない]{.yellow}
— グレーになり「いいえ」に投票。[**7つ中5つがまだ「はい」に
投票。**]{.yellow}

[**バー。**]{.accent} 寄与する仮説が減る → $x$ より**低い**バー。

[**なぜ速く落ちるか。**]{.accent} 脱落したのは**最小**の区間 — サイズ原理に
よれば事後確率の重みが**最大**の票。それを最初に失うことが、勾配が $x$ の
近くで急に減衰する理由。
:::
:::
::::
...
support) no longer contain y, so they gray out. 5 of 7 still vote. The
...
- [Welcome + Clusters walkthrough]{.lang-en .done}[ようこそ + クラスタ課題の説明]{.lang-ja .done}  [0:00]{.time}
- [The generalization problem]{.lang-en .done}[一般化の問題]{.lang-ja .done}  [0:10]{.time}
- [The Bayesian generalization framework + size principle]{.lang-en .done}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja .done}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en .highlight}[長方形ゲーム + 数当てゲーム]{.lang-ja .highlight}  [0:40]{.time}
- [Break]{.lang-en}[休憩]{.lang-ja}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en}[学生発表 — ショウヘイ]{.lang-ja}  [1:15]{.time}
- [No Free Lunch]{.lang-en}[ノーフリーランチ]{.lang-ja}  [1:40]{.time}
- [Hierarchical Bayes + close]{.lang-en}[階層ベイズ + まとめ]{.lang-ja}  [1:50]{.time}
...
## [The 1-D gradient — built from votes]{.lang-en}[1次元の勾配 — 投票から構築]{.lang-ja} {.smaller}

:::: {.columns .v-spread}
::: {.column width="56%"}
![](images/cc_1d_gradient.png){fig-align="center" width="100%"}
:::
::: {.column width="44%"}
::: {.lang-en}
The same posterior-weighted vote as the number-line construction — now with
**several examples** $X$.

[**Who votes.**]{.accent} Every interval that **contains all of $X$** is a
live hypothesis; a snugger interval is thicker (more posterior).

[**The gradient.**]{.accent} Sum the posterior of every interval that
contains $y$:
$$p(y \in C \mid X) = \sum_h \mathbf{1}[\,y \in h\,]\,p(h \mid X)$$

[Flat across the data, decaying outside — the gradient *is* the
posterior-weighted vote, one $y$ at a time.]{.yellow}
:::

::: {.lang-ja}
数直線の構築と同じ事後確率による投票 — 今度は**複数の例** $X$ で。

[**誰が投票するか。**]{.accent} $X$ の**すべてを含む**区間が生きた仮説;
ぴったりした区間ほど太い（事後確率が大きい）。

[**勾配。**]{.accent} $y$ を含むすべての区間の事後確率を合計:
$$p(y \in C \mid X) = \sum_h \mathbf{1}[\,y \in h\,]\,p(h \mid X)$$

[データ全体で平坦、外側で減衰 — 勾配*そのもの*が、$y$ を1点ずつ見た
事後確率による投票。]{.yellow}
:::
:::
::::
...
::: {.notes}
The continuous analog of the y = x / x+1 / x+2 walk-through, but with several
examples at once: each consistent interval votes with its posterior weight,
and the summed votes trace the generalization gradient. The plateau is exactly
the data range; outside it, intervals drop out and the vote falls. Same
equation as the discrete construction — only H changed from number-sets to
intervals. ~1.5 min.
:::
...
[$r$ — the range the data spans.]{.accent}

[$d$ — how far a rectangle extends past that range.]{.accent}
:::
...
(dot count) labeled. Lock in r and d here — the next two slides use them.
~2 min.
:::
...
::: {.lang-en}
[Tenenbaum (1999) ran this as a behavioral experiment.]{.accent}
...
is whether human d tracks the Bayesian prediction. d is the behavioral
...
::: {.column width="38%"}
::: {.lang-en}
[**Solid = human, dashed = model.**]{.accent} One color per $n$.

[**The human pattern.**]{.yellow} Fewer examples → generalize further ($n=2$
on top); $d$ rises with $r$ but **saturates**.

[**The model — likelihood only.**]{.accent} The size principle with a *flat*
(uninformative) prior. It captures the $n$-ordering…

[…but the curves run **straight** — the model **over-extends**, badly for
small $n$ and large $r$. It misses the human saturation.]{.yellow}
:::

::: {.lang-ja}
[**実線 = 人間、破線 = モデル。**]{.accent} 色は $n$ ごと。

[**人間のパターン。**]{.yellow} 例が少ない → 遠くまで一般化（$n=2$ が一番
上）; $d$ は $r$ とともに増えるが**飽和する**。

[**モデル — 尤度のみ。**]{.accent} *平坦な*（無情報）事前のサイズ原理。
$n$ の順序は捉えるが…

[…曲線は**直線的**に伸びる — モデルは**拡張しすぎ**、特に小さい $n$ と
大きい $r$ で。人間の飽和を捉えられない。]{.yellow}
:::
:::
::::

::: {.notes}
The Tenenbaum (1999) rectangle result, recreated dark-theme. Human curves
(solid) are from Fig 3a (avg of 6 subjects); model curves (dashed) are the
Bayesian learner with the UNINFORMATIVE prior, computed from the paper's
Eq (3) — which is exactly LINEAR in r. The point of THIS slide: the size
principle alone gets the n-ordering right (n=2 above n=50) but the
uninformative-prior model over-extends — the dashed n=2 line runs to d ~ 3.3
while people saturate near d ~ 1. That gap is what the next slide fixes.

Standing convention: when discussing a paper, always show human AND model
on the same axes — never a model curve alone, never a "the model reproduces
it" claim without the human curve beside it. ~2 min.
:::
...
::: {.column width="48%"}
::: {.lang-en}
A flat prior lets the rectangle run **straight** — it **over-extends**. The
fix: a prior that makes large rectangles less likely.
...
::: {.lang-ja}
平坦な事前では長方形が**直線的**に伸びる — **拡張しすぎる**。修正: 大きい
長方形を起こりにくくする事前。
...
ANTICIPATED STUDENT QUESTION — how is the scale parameter determined? The
exponential prior has one free parameter: equivalently the rate λ, or the
mean scale 1/λ (Tenenbaum writes it as σ, the prior's expected rectangle
size). It is NOT learned from the n dots of a single trial — those go into
the likelihood. It is a property of the prior, fixed across trials. In the
1999 paper σ is fit once to the average human data (he reports σ = 5 units
in the 24-unit window gives an excellent fit) — i.e. it is calibrated to the
population, then held constant. Conceptually it encodes prior experience
with the scale of "healthy-level" rectangles. The principled move (Week 4's
hierarchical-Bayes block) is to put a hyperprior on σ and infer it too — then
the scale is learned from many concepts rather than hand-set. Have this ready
but do not lecture it unprompted.
:::

## [With the prior — the fit]{.lang-en}[事前を入れると — 適合]{.lang-ja} {.smaller}

:::: {.columns .v-spread}
::: {.column width="62%"}
![](images/tg_results_prior.png){fig-align="center" width="100%"}
:::
::: {.column width="38%"}
::: {.lang-en}
[**Same axes, same data**]{.accent} — now the model carries the exponential
prior over rectangle size.

[**The dashed curves bend.**]{.yellow} The straight, over-extending lines
collapse onto the human curves — $d$ now **saturates** with $r$, just as
people do.

Likelihood (size principle) **and** prior together — neither alone — give
the fit.

[Same lesson for every paper: lay the **model on top of the human data** and
read off where it bends.]{.accent}
:::

::: {.lang-ja}
[**同じ軸、同じデータ**]{.accent} — 今度はモデルが長方形サイズに対する
指数事前を持つ。

[**破線の曲線が曲がる。**]{.yellow} 直線的に拡張しすぎていた線が人間の曲線
へと収束する — $d$ は今や $r$ とともに**飽和する**、人間と同じように。

尤度（サイズ原理）**と**事前の両方で — どちらか一方では不十分 — 適合が
得られる。

[どの論文でも同じ教訓: **モデルを人間データに重ねて**、どこで曲がるかを
読み取る。]{.accent}
:::
:::
::::
...
::: {.notes}
The payoff slide. Same Tenenbaum (1999) experiment, same human curves; the
model is now the expected-size-prior learner, computed from the paper's
Eq (4) with σ = 5. The dashed curves go from linear (previous slide) to
saturating, landing essentially on the human data — the "excellent fit" the
paper reports (its Fig 3d). The conceptual close: the size principle gives
the n-ordering, the prior supplies the nonlinearity; you need both. Reinforce
the standing convention — model always shown against human data. ~1.5 min.
:::
...
renormalized prior. In THIS example both hypotheses contain every example, so
nothing is eliminated and the posterior sits at 0.5 / 0.5 (and the 0.5 is an
artifact of two hypotheses + a flat prior — five survivors would give 0.2
...
- [The generalization problem]{.lang-en .done}[一般化の問題]{.lang-ja .done}  [0:10]{.time}
- [The Bayesian generalization framework + size principle]{.lang-en .done}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja .done}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en .done}[長方形ゲーム + 数当てゲーム]{.lang-ja .done}  [0:40]{.time}
- [Break]{.lang-en .highlight}[休憩]{.lang-ja .highlight}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en}[学生発表 — ショウヘイ]{.lang-ja}  [1:15]{.time}
- [No Free Lunch]{.lang-en}[ノーフリーランチ]{.lang-ja}  [1:40]{.time}
- [Hierarchical Bayes + close]{.lang-en}[階層ベイズ + まとめ]{.lang-ja}  [1:50]{.time}
...
## [Break]{.lang-en}[休憩]{.lang-ja} {.section-break .photo-break background-image="images/calliejimMay2025.jpg" background-size="cover" background-position="center"}

::: {.notes}
5 min break. The background is Callie and Jim (May 2025). After the break:
one bridge slide, then Shohei presents.
:::
...
- [The Bayesian generalization framework + size principle]{.lang-en .done}[ベイズ一般化の枠組み + サイズ原理]{.lang-ja .done}  [0:18]{.time}
- [Rectangle game + number game]{.lang-en .done}[長方形ゲーム + 数当てゲーム]{.lang-ja .done}  [0:40]{.time}
- [Break]{.lang-en .done}[休憩]{.lang-ja .done}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en .done}[学生発表 — ショウヘイ]{.lang-ja .done}  [1:15]{.time}
- [No Free Lunch]{.lang-en .highlight}[ノーフリーランチ]{.lang-ja .highlight}  [1:40]{.time}
- [Hierarchical Bayes + close]{.lang-en}[階層ベイズ + まとめ]{.lang-ja}  [1:50]{.time}
...
## [The No Free Lunch theorem]{.lang-en}[ノーフリーランチ定理]{.lang-ja} {.smaller}

::: {.lang-en}
[Wolpert (1996)]{.accent} — averaged over **all possible worlds**, no learning
algorithm beats any other. Here is *why*, concretely.
:::

::: {.lang-ja}
[Wolpert (1996)]{.accent} — **あらゆる可能な世界**にわたって平均すると、
どの学習アルゴリズムも他のどれにも勝らない。具体的に、その*理由*。
:::

:::: {.columns .v-spread}
::: {.column width="50%"}
::: {.lang-en}
[**The task.**]{.accent} You see the bits $\,0, 1\,$ and must predict $x_3$.

[**Your rule.**]{.accent} Say it predicts $x_3 = 0$. In the world
$\,0,1,\mathbf{0}\,$ it is **right**.

[**The mirror world.**]{.yellow} But the world $\,0,1,\mathbf{1}\,$ is *just
as possible* — same data $0,1$, opposite answer. There your rule is **wrong**.
:::

::: {.lang-ja}
[**課題。**]{.accent} ビット列 $\,0, 1\,$ を見て $x_3$ を予測する。

[**あなたの規則。**]{.accent} $x_3 = 0$ と予測するとしよう。世界
$\,0,1,\mathbf{0}\,$ では**正解**。

[**鏡の世界。**]{.yellow} だが世界 $\,0,1,\mathbf{1}\,$ も*同じくらい可能* —
同じデータ $0,1$、逆の答え。そこではあなたの規則は**不正解**。
:::
:::

::: {.column width="50%"}
::: {.lang-en}
[**They pair off.**]{.accent} Every world where your rule scores a point has a
mirror world — identical data, flipped continuation — where it loses one.

[**Sum the pair.**]{.accent} Right + wrong $= 1$ hit out of $2$. Average over
*all* worlds: every algorithm, every rule, scores **exactly $1/2$**.

[No rule can win the average, because the data $0,1$ says **nothing** about
$x_3$ until you assume some worlds are more likely than others.]{.yellow}
:::

::: {.lang-ja}
[**ペアになる。**]{.accent} あなたの規則が得点する世界には必ず鏡の世界が
ある — 同じデータ、反転した継続 — そこでは失点する。

[**ペアを合計。**]{.accent} 正解 + 不正解 $= 2$ 回中 $1$ 回的中。*すべて*の
世界で平均すると、どのアルゴリズムも、どの規則も、**ちょうど $1/2$**。

[どの規則も平均で勝てない。データ $0,1$ は、ある世界が他より起こりやすいと
仮定するまで $x_3$ について**何も**語らないから。]{.yellow}
:::
:::
::::

::: {.notes}
Walk the concrete instance, slowly — this is the intuition pump for the whole
NFL → prior payoff.

The setup: data is the two bits 0,1; predict the third bit. There are exactly
two continuations, 0,1,0 and 0,1,1, and with a flat prior over worlds they are
equally likely. Whatever your rule outputs, it is right in one of those two
worlds and wrong in the other. Worlds pair off: for every world your rule
gets right, the mirror world (same observed data, opposite continuation)
cancels it. So averaged over all worlds the score is exactly 1/2 — and this is
true of EVERY rule, including the cleverest one, including "always predict the
majority bit." That is the No Free Lunch theorem made concrete.

The escape hatch (next slide): the only way out is to drop the flat prior —
to assert that 0,1,0 is more likely than 0,1,1 (or vice versa). That
assertion IS the inductive bias. NFL doesn't say learning is impossible; it
says learning is impossible WITHOUT a prior. ~2.5 min.
:::
...
- [Break]{.lang-en .done}[休憩]{.lang-ja .done}  [1:08]{.time}
- [Student presentation — Shohei]{.lang-en .done}[学生発表 — ショウヘイ]{.lang-ja .done}  [1:15]{.time}
- [No Free Lunch]{.lang-en .done}[ノーフリーランチ]{.lang-ja .done}  [1:40]{.time}
- [Hierarchical Bayes + close]{.lang-en .highlight}[階層ベイズ + まとめ]{.lang-ja .highlight}  [1:50]{.time}
