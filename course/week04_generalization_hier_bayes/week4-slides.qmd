...
[**all 7 of 7 vote "yes"**]{.yellow}. None drops out.

[**The bar.**]{.accent} The vote at $x$ sums *all* the posterior weight —
this is the **tallest bar the gradient can have**. Generalization to the
datum itself is near-certain.
:::
...
[**誰が投票するか。**]{.accent} すべての仮説は $x$ を含むように作られた
ので、[**7つ中7つすべてが「はい」に投票**]{.yellow}。脱落なし。

[**バー。**]{.accent} $x$ での投票は*すべて*の事後確率を合計 — これが
**勾配が取りうる最も高いバー**。データ自体への一般化はほぼ確実。
:::
:::
::::
...
::: {.column width="46%"}
::: {.lang-en}
[**The question.**]{.accent} Move one step out: does the property generalize
to $y = x + 1$?

[**Who drops.**]{.accent} [The two smallest intervals no longer reach
$y$]{.yellow} — they grey out. [**5 of 7 still vote.**]{.yellow}

[**The bar.**]{.accent} Fewer hypotheses contribute → a **shorter** bar than
at $x$. And note *which* dropped: the **smallest, most-likely** intervals.
Losing the heavy-weight votes is why the gradient falls off quickly.
:::

::: {.lang-ja}
[**問い。**]{.accent} 1歩外へ: 性質は $y = x + 1$ に一般化するか?

[**誰が脱落するか。**]{.accent} [最小の2区間はもう $y$ に届かない]{.yellow}
— グレーになる。[**7つ中5つがまだ投票。**]{.yellow}

[**バー。**]{.accent} 寄与する仮説が減る → $x$ より**低い**バー。しかも
脱落したのは**最小・最尤**の区間。重みの大きい票を失うことが、勾配が速く
落ちる理由。
:::
:::
::::
