...
\begin{enumerate}
\item {\bf Good proposal vs.\ bad proposal.} Here $N_{\mathrm{eff}}$ measures how well a proposal $q$ {\em covers the whole target} $p = N(0,1)$ --- a $q$ that matches $p$ everywhere gives even weights and $N_{\mathrm{eff}} \approx T$; a $q$ that misses much of $p$'s mass gives a few dominant weights and a small $N_{\mathrm{eff}}$. Using the importance-sampling weights in Eq.~\eqref{eq:ess}, compute $N_{\mathrm{eff}}$ for two proposals, both targeting $p = N(0,1)$:
\begin{enumerate}
\item[(i)] a {\em good} proposal that overlaps $p$ well --- for example $q = N(0, 1.5^2)$ (same center as $p$, a bit wider); and
\item[(ii)] a {\em bad} proposal that overlaps $p$ poorly --- for example $q = N(4,1)$, shifted well off $p$'s mass. (A much narrower $q$, such as $N(0, 0.5^2)$, also lowers $N_{\mathrm{eff}}$, though less drastically than a far-shifted one.)
\end{enumerate}
Use $T = 1000$ for both. Report the two effective sample sizes (with $q=N(0,1.5^2)$ versus $q=N(4,1)$ you should see the good $q$ give an $N_{\mathrm{eff}}$ in the high hundreds and the far-shifted bad $q$ give one in the single digits). In 2--3 sentences, explain why the bad proposal gives a much smaller $N_{\mathrm{eff}}$ --- what does a small $N_{\mathrm{eff}}$ tell you about the weights, and how does this connect to the weight-variance figure from lecture (good $q$ $\Rightarrow$ even weights $\Rightarrow$ high $N_{\mathrm{eff}}$; bad $q$ $\Rightarrow$ a few weights dominate $\Rightarrow$ low $N_{\mathrm{eff}}$)? {\em (Note: do not use the $q=N(2,1)$ from Problem 1 here --- that proposal was tuned for the {\em tail} $X>2$, not for covering all of $p$, and it is exactly the subject of part (b).)}
