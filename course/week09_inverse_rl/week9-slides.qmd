...
## [Reading a mind *is* inverse planning]{.lang-en}[心を読むこと*は*逆プランニング]{.lang-ja}
...
[Theory of mind is **your own decision theory, run backwards** on someone else — the same inversion as the last block, now over *minds*. Reviewed as "ToM = inverse RL" (Jara-Ettinger 2019).]{.yellow}
:::
::: {.lang-ja}
...
[心の理論とは、**自分の意思決定理論を**他者に**逆向きに**走らせること — 前のブロックと同じ反転、対象が*心*になっただけ。「ToM = 逆強化学習」と総説（Jara-Ettinger 2019）。]{.yellow}
:::
:::
::: {.column width="56%"}
...
## [Theory of mind & autism — and a reframe]{.lang-en}[心の理論と自閉症 — そして再考]{.lang-ja} {.smaller}
...
## [Showing as recursive inference]{.lang-en}[再帰的推論としての「見せる」]{.lang-ja}
...
Your target — the observer's belief — is **hidden to you**, so planning the demonstration is **itself a POMDP** whose hidden state is *the observer's belief*. Teaching = inverse planning, **one level up**.
:::
::: {.lang-ja}
...
あなたの狙う相手 — 観察者の信念 — は**あなたに隠れている**から、デモの計画**自体がPOMDP**であり、その隠れ状態は*観察者の信念*だ。教示＝一段上の逆プランニング。
:::
:::
::::
...
## [What the tests actually look like]{.lang-en}[テストの実際]{.lang-ja} {.smaller}
...
**Ullman's perturbation (2023)** — make the box **transparent** so Anne *can see in*. A mind that tracks beliefs says "Anne now knows." Models often **still report the old false belief** — a belief-*preserving* tweak flips success to failure. ✗
...
**Ullmanの摂動（2023）** — 箱を**透明**にしてアンが*中を見える*ようにする。信念を追う心なら「アンは今や知っている」と言う。モデルはしばしば**依然として古い誤信念を報告** — 信念を*保つ*変更で成功が失敗に。✗
