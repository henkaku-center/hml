...
::: {.column width="50%"}
::: {.lang-en}
- Each is a precise **conditional-probability equality** — exactly the objects from Part I.
- Fairness isn't vague: it's *this*, and you must choose *which*.

[Next: why no classifier can satisfy all three.]{.dim}
:::
::: {.lang-ja}
- それぞれが厳密な**条件付き確率の等式** — まさにPart Iの対象。
- 公平性は曖昧ではない：*これ*であり、*どれ*かを選ばねばならない。

[次：なぜ三つすべてを満たす分類器はないのか。]{.dim}
:::
:::
::::
...
## [Reveal]{.lang-en}[答え]{.lang-ja} {.poll-reveal .midbig}

[**B — equal accuracy can hide very unequal errors.**]{.lang-en .yellow}[**B — 等しい精度は非常に不均等な誤りを隠しうる。**]{.lang-ja .yellow}

::: {.lang-en}
- Two groups can share one accuracy number while one suffers all the false denials and the other all the false approvals.
- *Accuracy is not a fairness criterion* — name the conditional probability you care about.
:::
::: {.lang-ja}
- 二つの集団が一つの精度の数値を共有しつつ、一方が誤った却下をすべて、他方が誤った承認をすべて被りうる。
...
::: {.lang-en}
- A perturbation you can't even see flips the kiosk: tonkatsu → hamburger.
- The attack is training in reverse: the **gradient of the loss w.r.t. the input** (`jax.grad` at $\mathbf{x}$).
- In high dimensions the boundary is near almost every point — one tiny step crosses it.
:::
::: {.lang-ja}
- 見えないほどの摂動でキオスクが反転：トンカツ → ハンバーグ。
- 攻撃は訓練の逆走：**入力に関する損失の勾配**（`jax.grad` を $\mathbf{x}$ に）。
- 高次元では境界がほぼ全点の近くにある — 一歩で越える。
:::
