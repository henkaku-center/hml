...
- [ ] **Mirror the new `sds.scss` rules to lecture-plans.** Three additive rules were added to `sds-reveal/sds.scss`: `.reveal .columns.v-center` (vertical-centering modifier for two-column slides), `.reveal .columns.v-spread` (distributes a text column's blocks evenly top-to-bottom beside a figure — toggle-scoped so it doesn't out-specify `lang-toggle.css`'s `.lang-ja { display:none }`), and `.agenda li.done` / `.done` (dimmed completed-item style for "where are we" recap agendas). Per the cross-repo sync rule (CLAUDE.md), copy all three blocks into `lecture-plans/sds-reveal/sds.scss` so the two theme files stay byte-identical.
