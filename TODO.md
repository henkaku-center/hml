...
- [ ] **Mirror the new `sds.scss` rules to lecture-plans.** Four additive rules were added to `sds-reveal/sds.scss`: `.reveal .columns.v-center` (vertical-centering modifier for two-column slides), `.reveal .columns.v-spread` (distributes a text column's blocks evenly top-to-bottom beside a figure — toggle-scoped so it doesn't out-specify `lang-toggle.css`'s `.lang-ja { display:none }`), `.agenda li.done` / `.done` (dimmed completed-item style for "where are we" recap agendas), and `.section-break.photo-break` (a section-break with a photo background instead of the yellow frame — soft dark pill + text shadow keep the heading legible). Per the cross-repo sync rule (CLAUDE.md), copy all four blocks into `lecture-plans/sds-reveal/sds.scss` so the two theme files stay byte-identical.
