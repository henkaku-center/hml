...
- [ ] **Mirror the new `sds.scss` rules to lecture-plans.** Two additive rules were added to `sds-reveal/sds.scss` (2026-05-21): `.reveal .columns.v-center` (vertical-centering modifier for two-column slides) and `.agenda li.done` / `.done` (dimmed completed-item style for "where are we" recap agendas). Per the cross-repo sync rule (CLAUDE.md), copy both blocks into `lecture-plans/sds-reveal/sds.scss` so the two theme files stay byte-identical.
