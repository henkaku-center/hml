...
- **Chibany mascot IP: only the official, unaltered pose images are permitted — anywhere (lectures, textbook, GitHub Pages).** The approved assets are `chibanyplain.png` (standing) and `chibanylayingdown.png` (laying down); reuse those files as-is. **Never generate, composite, or draw a derivative expression or pose** (sad, crying, angry, injured, etc.) and never alter the artwork itself (recoloring, distorting, adding drawn-on features) — code may only do non-content operations like transparent-background extraction for compositing (see `_mascot()` in `course/week12_ethics_adversarial/make_figures.py`) or resizing/placement. A sad-expression derivative was created and shipped in an early Week 12 adversarial-example figure and had to be removed (fixed 2026-07-18: both panel bookends now reuse the plain pose) — check any new figure that places Chibany in an emotionally-charged scene (mistakes, failures, being fooled) for exactly this trap.
...
**Layout archetypes are chosen at outline time (Rule 0).** The layout scheme
(`SLIDE_VISUAL_QA.md` → "The layout scheme (v2)") defines the menu — big-figure,
split (columns), cards, bullets, statement, poll, build-up, widget — with a
content-shape → archetype decision table. The outline names each slide's
archetype the same way it names its figures; "title + text" is a choice, not a
default. The audit prints a per-deck layout-mix histogram and flags a
>55%-bullets/prose deck as MONO-LAYOUT.
...
- **A substantial figure stacked under a text block gets crushed to nothing — now theme-prevented, but the authoring rule stands.** The fill-the-slide flex used to let a tall bullet list starve a figure to a microscopic sliver (Week 9 "Build the inversion", Week 12 slide 8). The layout-scheme v2 theme (2026-07) makes figures the flex-GROW element with hard floors (150px default / 300px `.big-figure`) and `object-fit: contain`, so the failure now surfaces loudly as OVERFLOW/TINY-FIGURE instead of a silent crush. When that flag fires: trim the text to ≤2 caption lines, use a two-column split, or give the figure its own slide — NEVER shrink the figure or lower a floor. Pick the layout archetype at outline time (Rule 0 in `SLIDE_VISUAL_QA.md` "The layout scheme").
...
(f) **For any Quarto/RevealJS week, the fill audit was re-run against the FINAL render with ZERO unexplained flags.** Run `DECK=… OUT_DIR=… node scripts/audit_slide_fill.js --threshold 75`. The audit (i) reveals every `.fragment` before measuring (a flagged poll is a real defect, not the old hidden-fragment false positive), (ii) measures at the deck's EXACT logical size (960×540 — an enlarged viewport lets Reveal's zoom change text wrapping and hide true clips; the Week 12 slide-7 lesson), (iii) runs the `scrollHeight > clientHeight` clip test itself, and (iv) measures figures and prose directly: **TINY-FIGURE** (largest figure paints < 24% of stage), **SQUISHED-FIGURE** (aspect distortion), **PROSE-WALL** (a 4+-line paragraph), plus a per-deck layout-mix histogram (MONO-LAYOUT advisory > 55% bullets/prose). **The hard gates: no slide may genuinely CLIP, no figure may flag TINY/SQUISHED, no PROSE-WALL, and no content slide top-jammed below ~70% fill.** A slide that merely *fills to the bottom edge* (`fill≈100%`, no clip) is GOOD, not OVERFLOW. When a FIGURE slide overflows, the figure floor is working: fix the TEXT (trim to caption length, split to columns, own slide) — never shrink the figure. **A residual BOTTOM-GAP in the 70–75% band is allowed only with a per-slide note; a blanket "remaining flags are benign BOTTOM-GAP / just the polls / benign COLUMN-THIN" dismissal is forbidden** — that exact sentence is what let half-empty slides ship across Weeks 6–7.
